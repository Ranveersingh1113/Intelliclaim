...
# MODIFIED: Check for Gemini API Key
...
# --- Specialized Agents (Logic Updated for Gemini) ---
class GeminiClient:
    ...
    async def process(self, context: QueryContext) -> QueryContext:
        ...
        try:
            if search_query in self.retrieval_cache:
                ...
                docs = await asyncio.to_thread(self.vector_store.similarity_search, search_query, k=5)
...
# --- Main RAG System Orchestrator (Logic Updated for Gemini) ---
class IntelliClaimRAG:
    def __init__(self):
        self.llm = GeminiClient(os.getenv("GOOGLE_API_KEY"))
...
async def test_gemini():
    try:
        model = GeminiClient(os.getenv("GOOGLE_API_KEY"))
    ...
    try:
        ...
        async def generate_answer(questions_batch, vector_store, model, all_documents, doc_length, doc_hash):
            ...
            try:
                ...
                retriever = _HybridRetriever(vector_store, clause_map=CLAUSE_KEYWORDS)
                    ...
                    if raw_context and avg_faiss > 1.1 and not (expects_identifier or expects_article24_age or expects_article17_abolish):
                        ...
                        if cached_summary is None:
                            ...
                            try:
                                ...
                                context = summary_response.text.strip()
...
Respond as a JSON array of answers, like: ["Answer1", "Answer2", "Answer3"]"""
                ...
                for attempt in range(batch_max_retries):
                    try:
                        ...
                        print("Debug: Full Gemini response:", response.text)
